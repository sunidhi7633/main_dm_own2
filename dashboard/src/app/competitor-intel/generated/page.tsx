...
export default function GeneratedPage() {
  ...
  const fetchPosts = async () => {
    try {
      ...
      setPosts(Array.isArray(await r.json()) ? await r.json() : []);
  ...
  useEffect(() => {
    ...
    load();
  }, [brandFilter, platformFilter, statusFilter]);
  ...
  const submitAction = async () => {
    ...
    setSaving(true);
    try {
      const body: any = { action, content: editContent };
    ...
    <>
      ...
      <div style={{ padding: "28px 40px", maxWidth: 1400, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 20 }}>
          ...
          {brands.map(b => (
            <button key={b} className={`filter-btn${brandFilter === b ? " active" : ""}`} onClick={() => setBrandFilter(b)}>
                  ...
                  <div key={p.id} className={`gen-card${selected?.id === p.id ? " selected" : ""}`} onClick={() => openPost(p)}>
                    <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 12, color: "var(--muted)", lineHeight: 1.5, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" as any, overflow: "hidden" }}>
                          {p.content}
                        </div>
                      </div>
